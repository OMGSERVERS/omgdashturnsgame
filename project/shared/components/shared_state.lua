...
shared_state = {
	...
	create = function(self)
		local qualifier = shared_state.DEATH_MATCH
		local settings = {
			level = nil,
		}
		local state = {
		...
		return {
			qualifier = "shared_state",
			-- Methods
			init_state = function(instance, new_settings)
				print(os.date() .. " [SHARED_STATE] Initialize match state")
			...
			set_state = function(instance, new_settings, new_state, new_step)
				print(os.date() .. " [SHARED_STATE] Set match state, step=" .. tostring(new_step))
				print(os.date() .. " [SHARED_STATE] Settings:")
				pprint(new_settings)
				print(os.date() .. " [SHARED_STATE] State:")
				pprint(new_state)
			...
			is_death_match = function()
				return qualifier == shared_state.DEATH_MATCH
			end,
